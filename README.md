...
`api/`는 팀의 OpenAI 키를 뒤에 두고 공개 URL로 열립니다. 두 겹으로 막아뒀습니다.

- **CORS 기본값이 닫혀 있습니다.** `ALLOW_ORIGIN`을 설정하지 않으면 교차 출처 응답이 거부됩니다.
  Vercel 환경변수에 `ALLOW_ORIGIN=https://<프론트엔드 도메인>`을 반드시 넣으세요.
- **레이트리밋**(`api/_rate-limit.js`): IP당 분당 20회, 전체 분당 200회.
  CORS는 브라우저만 막고 `curl`은 못 막으므로 호출 자체에 상한을 뒀습니다.
  서버리스 인스턴스별 카운터라 엄밀한 상한이 아니라 폭주를 늦추는 브레이크입니다.
